...
//    private final String group_key;
    ...
    public P2PPeer(InetAddress server_address, int server_port, String group_id, String identifier) throws SocketException {
...
//        this.group_id = group_id;
    ...
    private void update_peers(Peer[] peers) {
        this.peers = peers;
    }

    private Peer[] get_peers() {
        return this.peers;
    }
    ...
    public void run() {
        ...
        Thread register_self_to_server = new Thread(P2PPeer.this::register);
        Thread thread_receive_msg = new Thread(P2PPeer.this::receive_msg);
        Thread thread_send_shit_to_peers = new Thread(P2PPeer.this::send_shit_to_peers);
        ...
        while (!this.socket.isClosed()) {
            try {
                ...
                if (this.packet.getAddress().equals(this.server_address) && this.packet.getPort() == this.server_port) {
                    ...
                    Peer[] peers = new Peer[json_peers.size()];
	...
	public static void main(String args[]) {
        ...
        if (args.length < 2) {
            System.err.println("run with command 'java P2PPeer <server_host>[:listening_port] <group_id>' " + bad());
            System.exit(-1);
        }
        ...
        try {
            (new P2PPeer(InetAddress.getByName(server_host), server_port, group_id, RandomName.pick())).run();
